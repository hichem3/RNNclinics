...
#Parameters set-up
###############################################################################
maxlen = 200 #cutoff reviews, raise to 500
training_samples = 500 #raise to 20000
val = 100 #raise to 5000
test=val
max_words = 2000 #vocabulary dimension, raise to 10000
...
#Functions set-up
###############################################################################
def one_step_attention(a, s_prev, rep):
    """
    ...
    s_prev = RepeatVector(rep)(s_prev)
    #print(s_prev.shape)
...
def model_building(Tx, Ty, n_a, n_s, human_vocab_size):
    ...
    #RMK: filters(32) is the number of features I want to extract, kernel_size(5),size of the sliding window
    X_red = Conv1D(32, 5, strides=1, activation='relu')(X)
    X_pool = MaxPooling1D(2)(X_red)
    X_red2 = Conv1D(32, 5, strides=1, activation='relu')(X_pool) #troubleshooting: don't call this 'X'
    #?x61x32 con 32,5,1,3 acc=0.81 or 0.82(batch=128)|| ?x21x64 con 64,7,2,2  acc 0.78 || 91x64 con 64,7,1,2 acc=0.79
    # 94x64 con 64,5,1,2 acc=0.78 || 94x128 con 128,5,1,2 acc=0.71 || 8x128 con 128,9,3,2 NO
    # 42x64 con 64,7,1,4 acc=0.77 || 94x32 con 32,5,1,2 acc=0.85!! Too trained maybe 
    
    
    rep = keras.backend.int_shape(X_red2)[1]
    
    # Step 1: Define your pre-attention Bi-LSTM. Remember to use return_sequences=True.
    #return_sequences: Whether to return the last output in the output sequence, or the full sequence.
    a = Bidirectional(LSTM(n_a, return_sequences=True))(X_red2) #ENCODER
    # Step 2: Iterate for Ty steps
    # Step 2.A: Perform one step of the attention mechanism to get back the context vector at step t
    for t in range(5): #I choose to have a many to many Tx-->10
        context = one_step_attention(a, s, rep) #output is [?,1,64]
...
model,deout,des = model_building(Tx, Ty, n_a, n_s, max_words)

opt = Adam(lr=0.01, beta_1=0.9, beta_2=0.999, decay=0.01)

model.compile(optimizer=opt,loss='binary_crossentropy',metrics=['acc'])

history = model.fit([X,s0,c0],Y,epochs=10,batch_size=32,validation_data=([x_val,s0val,c0val],y_val)) #increase batch size, epochs should be 10-17
